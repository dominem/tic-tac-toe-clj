...
* [`src/clj/tic_tac_toe_clj/`](src/clj/tic_tac_toe_clj/): Backend and middleware source files (Clojure,
[Compojure](https://github.com/weavejester/compojure))
* [`src/clj/tic_tac_toe_clj/`](src/clj/tic_tac_toe_clj/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/tic_tac_toe_clj/`](src/cljs/tic_tac_toe_clj/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/tic_tac_toe_clj/core.cljs): contains the SPA entry point, `init`
* [`test/cljs/tic_tac_toe_clj/`](test/cljs/tic_tac_toe_clj/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
7. Clone this repo and open a terminal in the `tic-tac-toe-clj` project root directory
...
in the [`src/clj/tic_tac_toe_clj/`](src/clj/tic_tac_toe_clj/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/tic_tac_toe_clj/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tic-tac-toe-clj.example
  (:require [tic-tac-toe-clj.config :as config])
...
standalone `tic-tac-toe-clj.jar`.
...
```sh
port=2000 java -jar target/tic-tac-toe-clj.jar
```
...
2. Set nodejs version, so that Heroku will install it during deployment:
    ```sh
    heroku config:set NODEJS_VERSION=12.18.3
    ```
